...
[![Use the template](https://img.shields.io/static/v1?label=&message=Click%20here%20to%20use%20this%20package%20as%20a%20template%20to%20start%20a%20new%20repo%20on%20GitHub&color=brightgreen&style=for-the-badge)](https://github.com/tomchen/pyGPU_optionpricer/generate)
...
(If you do not use GitHub, you can [download the archive of the example package](https://github.com/tomchen/pyGPU_optionpricer/archive/main.zip))

### Package, module name

Many use a same package and module name, you could definitely do that. But this example package and its module's names are different: `pyGPU_optionpricer` and `gpupricer`.

Open `pyGPU_optionpricer` folder with Visual Studio Code, <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>F</kbd> (Windows / Linux) or <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>F</kbd> (MacOS) to find all occurrences of both names and replace them with your package and module's names. Also remember to change the name of the folder **src/gpupricer**.
...
The package's version number `__version__` is in **src/gpupricer/\_\_init\_\_.py**. You may want to change that.
...
In **src/gpupricer/** (`gpupricer` should have been replaced by your module name) folder, rename **module1.py** and write your code in it. Add more module .py files if you need to.
...
If you add more files to the root directory (**pyGPU_optionpricer/**), you'll need to add your file to `check-manifest --ignore` list in **tox.ini**.
...
From `pyGPU_optionpricer` directory, run the following command, in order to generate production version for source distribution (sdist) in `dist` folder:
...
```bash
pip install dist/pyGPU_optionpricer-0.1.0.tar.gz
```

(You may need to uninstall existing package first:

```bash
pip uninstall pyGPU_optionpricer
```
...
Modify the version number `__version__` in **src\gpupricer\_\_init\_\_.py**.
